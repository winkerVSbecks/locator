angular.module('locator')
  .factory('reverseGeocoder', ['$document', '$q',
    function($document, $q) {
      ...
      service.geocodeAndFilter = function(location, filter) {
        var deferred = $q.defer();
        if (!location)
          deferred.reject('You need to provide LatLng');
        else
        ...
        service.geocoder.geocode({
          ...
          if (status !== google.maps.GeocoderStatus.OK) {
            ...
            var result;
            angular.forEach(results, function(address_component, i) {
              if (address_component.types[0] === filter && !result) {
